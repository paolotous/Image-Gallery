...
def upload_image(my_img):
    ...
    if os.path.isfile(root.filename):
        shutil.copy(root.filename, "../images")
        head, tail = os.path.split(root.filename)
        new_name = "../images/"+tail
...
def load_images(my_img, my_img_files):
    ...
    for file in os.listdir(directory):
        if file.endswith(".jpg") or file.endswith(".png"):
            my_img_files.append(Image.open("../images/"+file))
...
def delete_image(my_img, my_img_files, index):
    for file in my_img_files:
        print(file.filename)
    os.remove("../images/"+my_img_files[index].filename)
...
root.iconbitmap('D:\George Brown\Ampeross.ico')
